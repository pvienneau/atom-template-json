...
export default class GenerationActions{
    ...
    _firstName() {
        if (!(index = this._getIndex('firstName'))) {
            index = this.random(0, firstNames.length -1);
    ...
    oneOf(...collection) {
        return `"${this._oneOf(...collection)}"`;
    }

    _oneOf(...collection) {
        if (!collection || !collection.length) return '';

        if (!(index = this._getIndex('oneOf'))) {
            index = this.random(0, collection.length -1);
            this._setIndex('oneOf', index);
        }
