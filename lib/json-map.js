...
const templateJSONMap = {
    ...
    objectValue: (schema) => (schema.quotedStrictString() && schema.colon() && schema.value() && ((schema.comma() && schema.objectValue())|| schema.empty())) || schema.empty(),
    ...
    literal: (schema) => {return (schema.integer() || schema.quotedString() || schema.boolean() || schema.null())},
    integer: (schema) => '-?[0-9]+(\\.[0-9]*)?(e[0-9]+)?',
    quotedStrictString: (schema) => schema.doubleQuote() && schema.strictString() && schema.doubleQuote(),
    strictString: (schema) => '(\\\\"|[^"])*',
    quotedString: (schema) => schema.doubleQuote() && schema.string() && schema.doubleQuote(),
    ...
    //---------
    expression: (schema) => schema.doubleCurlyLeft(clearEatenInput) && schema.function(clearEatenInput) && schema.doubleCurlyRight(clearEatenInput),
    ...
    parameter: (schema) => schema.integer(),
};
